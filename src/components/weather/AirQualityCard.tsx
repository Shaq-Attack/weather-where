...
export function AirQualityCard({ lat, lon }: AirQualityCardProps) {
    ...
    const calculatePM25AQI = (concentration: number): number => {
      ...
      for (const bp of breakpoints) {
        if (concentration >= bp.cLo && concentration <= bp.cHi) {
          return Math.round(
            ((bp.iHi - bp.iLo) / (bp.cHi - bp.cLo)) * (concentration - bp.cLo) +
              bp.iLo,
          );
        }
      }
  ...
  const convertAirQualityData = (data: AirPollutionData): AirQualityData => {
    ...
    if (calculatedAQI <= 50) {
      ...
      color = "#ffff00";
      healthAdvice =
        "Air quality is acceptable. Sensitive individuals should consider limiting outdoor activities.";
      ...
      color = "#ff0000";
      healthAdvice =
        "Everyone should limit outdoor activities. Wear a mask if going outside.";
    } else if (calculatedAQI <= 300) {
      level = "Very Unhealthy";
      color = "#8f3f97";
      healthAdvice =
        "Avoid outdoor activities. Stay indoors with air purification if possible.";
    } else {
      level = "Hazardous";
      color = "#7e0023";
      healthAdvice =
  ...
  if (loading && !airQuality) {
    return (
      <LoadingCard message="Loading air quality data..." minHeight="300px" />
    );
  }

  if (!airQuality) {
    return (
      <ErrorCard message="No air quality data available. Location coordinates are required." />
    );
  }
  ...
  return (
    <>
      ...
      >
        ...
        >
          <CardTitle style={headerStyles.title}>🌬️ Air Quality Index</CardTitle>
        </CardHeader>
        ...
        <CardBody style={{ ...bodyStyles.base, height: "auto" }}>
          ...
          >
            <strong>Note:</strong> AQI calculated using US EPA standards based
            on pollutant concentrations from OpenWeatherMap API.
          </div>
