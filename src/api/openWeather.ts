...
export async function fetchForecast(
...
): Promise<FiveDay3HourResponse> {
  const API_KEY = import.meta.env.VITE_OPENWEATHER_API_KEY;

  if (!API_KEY) {
    throw new Error(
      "OpenWeather API key is not configured. Please check your .env file.",
    );
  }
  ...
  if (!res.ok) {
    if (res.status === 401) {
      throw new Error(
        "Invalid API key. Please check your OpenWeather API key in the .env file.",
      );
    }
...
export function convertToDaily(data: FiveDay3HourResponse): ForecastDay[] {
  ...
  return Object.values(dailyData).map((day: any) => ({
    ...
    temp: {
      ...
      max: day.temp_max,
      day:
        day.temps.reduce((sum: number, temp: number) => sum + temp, 0) /
        day.temps.length,
    },
    weather: [day.weather],
...
export function getCurrentUVIndex(uvData: UVIndexData): number {
  ...
  const localTime = new Date(
    now.getTime() + offsetMs + now.getTimezoneOffset() * 60000,
  );
...
export function getTodayPeakUV(uvData: UVIndexData): {
  peakTime: string;
  peakValue: number;
} {
  // Create a date object in the API's timezone
